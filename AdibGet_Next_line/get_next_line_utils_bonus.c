...
/*   Updated: 2024/12/02 15:00:26 by yadib            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin(char const *s1, char const *s2)
{
	...
	s1_len = ft_strlen(s1);
	total_leng = s1_len + ft_strlen(s2) + 1;
	concatenation_str = malloc(sizeof(char) * total_leng);
	if (concatenation_str == NULL)
	{
		free((char *)s1);
		return (NULL);
	}
	ft_strlcpy(concatenation_str, s1, (s1_len + 1));
